...
\textit{We add a nonzero constant $c$ in front of the expression and we also changed the expression of the leading monomial LM($f$) (note that we changed LT($f$) into LM($f$)). \TODO{ keep $\rightarrow$ We also added a short remark below Equation \eqref{eq:def_mon_order} to explain why later, we assume that $c=1$ while dealing with leading monomial (i.e. in Section \ref{subsection : Upper_bound_Goppa-like}).} }
...
\textit{The definition of $G$ has been added right before Definition \ref{def:Goppa--like_AG_code}.}
