...
\mathieu{MATHIEU : je met mes modif/question en marron. Les ajouts sont en noir.}
...
Over the past forty years, there were many attempts in replacing the family of binary Goppa codes by other structured families of codes to reduce the key size. \mathieu{je commence l'histoire sur les codes AG, je sais pas si ça vaut le coup de parler des autres tests ?}
In 1996, Janwa and Moreno \cite{JM96} proposed to use AG codes, concatenation or subfield subcode of these codes. As for concatened ones, they were broken by Sendrier \cite{Sen94}. For AG codes, Faure and Minder proposed an attack in \cite{FM08,Min07,Fau09} for codes on genus $\geq 2$ curves. The scheme based on AG codes have been completely broken by Couvreur, Marquez-Corbella and Pellikaan \cite{CMR17} who proposed a filtration based attack on AG codes for any genus. This leads to consider the ladt proposition, that is Subfield Subcode of AG codes (SSAG in short). In fact, as noticed in \cite{CMR17}, the square and the 2-closure of an SSAG code usually differ from the corresponding AG code, meaning that they resist the attack by filtration. Moreover, some of theses codes have a good designed minimum distance: see for example \cite{Cou14} for the construction of Cartier codes. 
...
In this paper, we introduce a specific king of SSAG code, namely Goppa--like AG codes. The idea of this construction is to immitate the role of the Goppa polynomial in the case of classical Goppa codes by a well-chosen rational function on the curve, meaning that we will consider divisors of the form $A+(g)$, where $A\in \Div(\calX)$ and $g \in \fqm(\calX)$. A Goppa--like AG code is then defined as the subfield subcode over $\fq$ of $C_{\calL}(\calX,P,A+(g))^{\perp}$. This construction coincides with classical Goppa codes in the genus 0 case, since the latter codes are defined as the subfield subcode of the dual of some GRS code. Provided that the curve $\calX$ allows to implement these codes fast enough, this generalization might be of interest. This motivates this work of studying strucural attacks on Goppa--like AG codes. 
