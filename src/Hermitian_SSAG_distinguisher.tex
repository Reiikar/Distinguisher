...
\paragraph{Relation with Cartier codes.} Cartier codes \cite{Cou14} are also defined as a geometric realisation of Goppa codes, since well-known properties on Goppa codes naturally extend to them.
...
\subsection{Cartier codes}
\TODO{Bouger ça avant même de parler de codes sur les courbes $C_{a,b}$}
...
Set $\calC_1=C_{\calL}(\calX,\mathcal{P},\lfloor \frac{G}{q} \rfloor) $. By Riemann--Roch theorem, $\ell(\lfloor \frac{G}{q} \rfloor)\geq 1$.  Then every non--zero codeword $\mathbf{c} \in  \calC_1 \subset \calC$ satisfies $\mathbf{c} \in \calC \cap \calC^q$, hence $\mathbf{c}^{q^i} \in \calC^{q^i} \cap \calC^{q^{i+1}}$ for every $i \in \set{0,\dots,\lfloor \frac{m}{2}\rfloor -1}$. This implies that the code $\calC \star \calC_1^{q^i}$ lies in the intersection of $\calC \star \calC^{q^i}$ and $\calC \star \calC^{q^{i+1}}$.
