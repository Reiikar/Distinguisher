...
\begin{remark}
	\TODO{reformuler}
The property above for $i=0$ follows from Proposition \ref{prop:mumford_result}. To the best of our knowledge, there is no sufficient criterion for the equality in the literature for $i \geq 1$. From our experiments, it may happen that $\ell(G)^2 > \ell((q^i+1)G)$ without the equality holding, which means that the generators of $\calL(G)\cdot \calL(G)^{q^i}$ may be linearly dependent. 
\end{remark}
