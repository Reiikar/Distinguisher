...
\jade{J'ai simplifié l'énoncé ci-dessous, en virant les $\calL(G)$.}

\begin{lemma} \label{lem:weighted_division}
Take $i \geq 1$ and $s'>s \geq 0$. Let $f,\: g \in \calS$ such that $\degab{f} < s'(q^i+1)$ and $\degab{g}= s'$. Fix a set of points  $\calP \subset \calX_{a,b}(\F_{q^m})$ such that $\calP \cap \Supp(g) = \varnothing$.

Then there exists $f' \in \calR\left(g^{q^i-q^{i-1}+1}\right)$ such that the vectors  $\ev_{\calP}\left(\frac{f}{g^{q^i+1}}\right)$ and $\ev_{\calP}\left(\frac{f'}{g^{q^i+1}}\right) $ have the same trace over $\fq$.
\end{lemma}

\begin{proof}
By Proposition \ref{prop:div_grob}, we can write $f=f_1 g^{q^i-q^{i-1}+1} +f_2$ with
 $f_2 \in \calR\left(g^{q^i-q^{i-1}+1}\right)$ and $\degab{f_2} \leq \degab{f}$. Dividing by $g^{q^i+1}$, we get
 \[\frac{f}{g^{q^i+1}}= \frac{f_1}{g^{q^{i-1}}} +\frac{f_2}{g^{q^i+1}}.\]
 
\[ = \Tr{\frac{f_1^qg}{g^{q^i+1}}} +\Tr{\frac{f_2}{g^{q^i+1}}}. \]
