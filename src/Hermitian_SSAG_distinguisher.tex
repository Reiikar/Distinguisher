...
\jade{TODO: On ne peut pas baser Mceliece sur un seul code : à reformuler}
...
\jade{TODO : changer $\calO_{P_\infty}$ car c'est pas du tout ça.}
...
%By assumption, we have $\mathrm{LM}(g)=x^{\beta}y^{\alpha}$, with $\alpha \leq a-1$ and $\degab{g}=a\beta +b\alpha=s'$. 
%Thus, $\mathrm{LM}(g^{q^i-q^{i-1}+1})=x^{\beta (q^i-q^{i-1}+1)}y^{\alpha (q^i-q^{i-1}+1)}$. Since $\alpha (q^i-q^{i-1}+1)$ might be bigger that $a-1$, we perform the following euclidean division:
%$$\alpha (q^i-q^{i-1}+1) = \delta a + \alpha_i, \ 0 \leq \alpha_i \leq a-1.$$
%Using the equation of the curve, we can then suppose that $\mathrm{LM}(g^{q^i-q^{i-1}+1})=x^{\beta_i}y^{\alpha '}$, with 
