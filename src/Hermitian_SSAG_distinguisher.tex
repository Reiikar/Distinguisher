...
\begin{itemize}
	...
	\item if $2k < \frac{m}{2}$, $\calA^k= \Gamma' \sqcup \Delta'$ where $\Gamma$ is the set of integral points in the rectangle of vertices $O$, $P$, $Q'(i_{\calS^k}q^k,j_{\calA^k})$ and $R'(i_{\calS^k}q^k,0)$ and $\Delta'$ those in the non--convex polygon of vertices  $T'(i_{\calS^k}q^k-1,0)$, $U'(i_{\calS^k}q^k,j_{\calA^k})$, $V'(i_{\calA^k},0)$, $W'(i_{\calA^k}-(q^{2k}+1),q^{2k}+1)$ and $X'(i_{\calA^k}-(q^{2k}-1),q^{2k}-1)$. 
\end{itemize}
...
\item To prove that $\Gamma \subset \calB^k$, we will fix an abscissa $i$ of a point in $\Gamma$ and we will show that all points $(i,0),\cdots,(i,q_0-1)$ belong to $\calB^k$. By definition of $\Gamma$, we have $i < i_{\calS^k}q^k=(q_0-1)q^k$. Let us write
    ...
    \mathieu{Est-ce qu'on garde la Figure 3? j'ai décalé les petits triangles au max du bords droit du triangle.}
...
\end{scope}
	...
	\node[yshift=0.7em] at ($(0,0)!0.5!(E)+({\q*(\qO-2)},0)$) {$\calS^k_{(q_0-2,0)}$};
	\node[yshift=0.5em] at ($(0,0)!0.5!(E)+({\q*(\qO-2)},\q)$) {$\calS^k_{(q_0-2,1)}$};
...
which gives $\# (\Gamma \backslash  \calS^k_{(q_0-1,0)}) = q^{2k}-q^k$. Again, those points are in the path joining $T'(i_{\calS^k}q^k-1,0)$, $U'(i_{\calS^k}q^k,j_{\calA^k})$, $V'(i_{\calA^k},0)$, $W'(i_{\calA^k}-(q^{2k}+1),q^{2k}+1)$ and $X'(i_{\calA^k}-(q^{2k}-1),q^{2k}-1)$. Amoung them, the ones of the form
\[ (i_{\calA^k}-\mu,\mu) \ , \ \mathrm{where} \ \mu \in \{q^k,\cdots,q^{2k}-1\}\]
cannot be in $\calS^k_{(i_{\calS^k},0)}$. Then $\Delta \setminus  \calS^k_{(i_{\calS^k},0)}$ consists precisely in these points.
We write
\[\mu = \mu_1 q^k + \mu_2 \ , \ \mathrm{with} \ \mu_1,\mu_2 < q^k,\]
and we claim  \[(i_{\calA^k}-\mu,\mu)=q^k(i_{\calS^k}-\mu_1,\mu_1)+(i_{\calS^k}-\mu_2,\mu_2) \in \calS^k_{i_{\calS^k}-\mu_1,\mu_1},\]
which proves \eqref{eq:Delta'_Bigcup} and conclude the proof.
