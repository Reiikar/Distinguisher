...
\paragraph{Codes from $C_{a,b}$ curves.} Let $a,b$ be coprime integers. A $C_{a,b}$ curve over $\fqm$ is a curve $\calX_{a,b}$ having an irreducible, affine and non singular plane model with equation
...
\noindent These curves are well-studied, and we know that any curve $\calX_{a,b}$ defined by an equation as in \eqref{eq:equation_C_ab} as a unique point at infinity, denoted by $P_{\infty}$. Moreover, its genus is also known:
 $$\mathfrak{g}(\calX_{a,b}) := \mathfrak{g}_{a,b} = \dfrac{(a-1)(b-1)}{2}.$$
...
\noindent The terminology Goppa-like is motivated by the fact here, the function $g$ plays the role of the Goppa polynomial, adding randomness to the code.
...
$$ \Tr{\calC}^{\star 2} = (\Gamma(\calP,g)^{\perp})^{\star2} \subseteq \sum\limits_{i=0}^{\lfloor{m/2} \rfloor} \Tr{\calC\star \calC^{q^i}}.$$ 
...
\begin{proposition} \label{prop:bound_dim_using_inclusions}
    ...
    $$\dim_{\fq} (\Gamma(\calP,g)^{\perp})^{\star 2} \leq \binom{mk+1}{2} - \dfrac{m}{2}\left(k(k-1)(2e+1)-2s\left(\dfrac{q^{e+1}-1}{q-1}\right)\right).$$
\end{proposition}

\begin{proof}
    From Proposition \ref{prop:Tr_BoundSchurSquare} and Lemma \ref{lem:bound_dim_Tr(C*C^q^i)}, we have 
    \begin{align*}
        \dim_{\fq}(\Gamma(\calP,g)^{\perp})^{\star 2}
...
$$\dim_{\fq} (\Gamma(\calP,g)^{\perp})^{\star 2}\leq \binom{mk+1}{2} - \dfrac{m}{2}(k^2(2e^*+1)+k-2s'(q^{e^*}-q^{e^*-1}+1)). $$
\end{coro}
...
\noindent Given an AG code $\calC := C_\calL(\calX_{a,b},\calP,D)$, where $D = sP_\infty +(g)$ and $\degab{g} = s'>s\geq 2\mathfrak{g}_{a,b}-1$, we proved above that whenever $$s \geq (s'-s)q+2\mathfrak{g}_{a,b}-1,$$ we have 

\begin{equation} \label{eq:best_upper_bound}
\dim_{\fq} (\Gamma(\calP,g)^{\perp})^{\star 2} \leq \frac{m}{2}\left(2s'(q^{e^*}-q^{e^*-1}+1)+k^2(m-1-2e^*)  \right),
\end{equation}
