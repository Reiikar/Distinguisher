...
cryptosystem based on codes over larger fields. We investigated Goppa-like AG codes over finite fields with different characteristics by analysing the computational cost of the ISD algorithm for parameters relevant to post-quantum cryptography. In the table below, we provided parameters that resist the Goppa-like AG code distinguisher and improve key sizes when compared to the subfield subcodes of 1-point Hermitian codes parameters reported in \cite[Table~3]{EKN21crypto}.
...
\begin{table}[h]
	\begin{center}
		...
		\end{tabular}
		\caption{Goppa-Like Hermitian codes parameters $\Gamma(\calP,sP_\infty,g)$ over $\fq$} 
	\end{center}
\end{table}
...
This work was funded in part by the grant ANR-21-CE39-0009-BARRACUDA from the French National Research Agency. The first and the third authors are supported by the French government ‘‘Investissements d’Avenir" program ANR-11-LABX-0020-01.
