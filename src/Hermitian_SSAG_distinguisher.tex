...
Moreover, if $m$ is even, 
	\begin{equation} \label{eq:dim_m/2} \dim_{\fq} \Tr{\calC\star \calC^{q^{\lfloor{m/2} \rfloor}}} \leq \frac{m}{2} (\dim_{\fqm}\calC)^2.
\end{equation}
\end{proposition}
...
\subsubsection{Relation with Cartier codes.} Cartier codes \cite{Cou14} are also defined as a geometric realisation of Goppa codes, since well-known properties on Goppa codes naturally extend to them.
...
\end{equation}
 Above, $i(G_1)$ stands for the index of speciality of $G_1$ (see \cite[Definition 1.6.10]{Sti09}). By the Riemann--Roch theorem, if $\deg(G_1) > 2\mathfrak{g}-2$, then $i(G_1) =0.$ The biggest divisor (with respect to the degree) satisfying \eqref{eq:divisor_G_1} is given by 
...
\subsubsection{Why imposing $g \notin \calL(D)$?}

In the definition of Goppa--like AG code, we ask for the function $g$ to lie outside the Riemann--space $\calL(D)$. By doing so, we cannot see all alternant codes as Goppa--like AG codes on $\calX=\PP^1$. However, to make these codes resistant to a distinguished based on the square of their dual, it is crucial to make sure that the dimension of $\Tr{\calC}^{\star 2}$ is not abnormally small compared to the typical value given in Corollary \ref{coro:first_bound_square_of_trace}. 

First, if the function $g$ lied in $\calL(D)$, then the code $\calC$ would contain the evaluation of the constant function $1=\frac{g}{g}$, which is the unit vector $(1,\dots,1)$. In this case, the vector $(1,\dots,1)$ would belong to $\calC^{q^i}$ for every $i \in \{0,\dots, \lfloor{m/2} \rfloor\}$ and each term in the sum on the right hand--side would contain a copy $\Tr{\calC}$. This non--trivial intersection between the codes $\Tr{\calC\star \calC^{q^i}}$ would contribute with a negative term in the above bound.

Secondly, if the function $g$ belonged to $\calL(D)$, then the divisor $D+(g)$ would be effective and for every $i \geq 0$. This would imply the inclusion $\calL((q^i+1)D) \subset \calL((q^{i+1}+1)D)$. Therefore, in the proof of Proposition \ref{prop:bound_dim_using_inclusions}, when bounding from above the dimension of the sum by the sum of the dimensions of the trace codes, we would have no chance to get a sharp bound.

\subsubsection{Sharpness of the bound}

\TODO{Réécrire sous forme de lemme + Exemple}

As recalled in Section \ref{subsec:def-Goppa}, Goppa--like AG codes coincide with Cartier code as soon as $\deg \lfloor \frac{G}{q} \rfloor > 2 \mathfrak{g}-2$ (see Equation \eqref{eq:G/q} for the definition of the divisor $\lfloor \frac{G}{q} \rfloor$).

Set $\calC_1=C_{\calL}(\calX,\mathcal{P},\lfloor \frac{G}{q} \rfloor) $. By Riemann--Roch theorem, $\ell(\lfloor \frac{G}{q} \rfloor)\geq 1$, which entails that $\dim \calC_1 \geq 1$.  Then every non--zero codeword $\mathbf{c} \in  \calC_1 \subset \calC$ satisfies $\mathbf{c} \in \calC \cap \calC^q$, hence $\mathbf{c}^{q^i} \in \calC^{q^i} \cap \calC^{q^{i+1}}$ for every $i \in \set{0,\dots,\lfloor \frac{m}{2}\rfloor -1}$. This implies that the code $\calC \star \calC_1^{q^i}$ lies in the intersection of $\calC \star \calC^{q^i}$ and $\calC \star \calC^{q^{i+1}}$.
