...
The following Lemma gives known bounds linked to the Shur product of codes.
...
\begin{thm}[Delsarte] \label{th:delsarte} \TODO{ref}
Let $\calC$ be a linear code. Then
...
The following Lemma gives known bounds linked to the star product of codes.
	...
	\begin{prop1} [\cite{rocco} Corollary 16]\label{1st bound square of trace}
		...
		\begin{equation} \label{eq:bbbound}
...
\begin{thm} \label{Tr_BoundSchurSquare}
		\jade{Ce sera défini avant : With usual definitions of the trace and the $\star$-product of codes,}
		 we have 
...
\textcolor{blue}{
\begin{thm} \label{th:Trace_shur_square_AG}
With usual definitions of the trace and the $\star$-product of codes, we have 
...
where $f :=\lfloor\log_q(r)\rfloor $, or even without the Trace operator in the case of RS-codes. This allow them to use more efficiently Thereom \ref to get a better bound. \\
...
also holds (with traces as well), and can be used to refine the estimation of the dimension of $\Tr(\calC)^{\star2}$ by rewriting Theorem \ref{th:Trace_shur_square_AG} as follows:
...
Note that we proved the result only for powers of $q_0$ and not on all powers of $q$, which is supposed to be the integers of interest in Theorem \ref{th:Trace_shur_square_AG}. More especially, the result above is only interesting in the case $i=1$ here, since the sum on the right hand-side of Theorem \ref{th:Trace_shur_square_AG} runs from the power $q^0$ to $q^{\frac{m}{2} }:=q_0$. \\
...
which lie on the path \jade{joining $U'$ and $V'$}. %
%
...
\begin{proof}
$(i)$ is a consequence of Proposition \ref{result_with_valuations} in the case $i=1$ (note that the result also holds without the Trace operator), while $(ii)$ uses Proposition \ref{powers_of_q's_case} several times. In both cases, we get a result on $\Tr{\calC}^{\star 2}$ by using Theorem \ref{th:Trace_shur_square_AG}.
\end{proof}
