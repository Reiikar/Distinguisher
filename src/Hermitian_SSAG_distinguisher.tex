...
\begin{proof}
From Proposition \ref{prop:dim_M_i's}, \textit{2)}, we know that the condition on $s$ and $s'$ implies that $M_{i^*}(s,g) = R(g^{q^{i^*}-q^{i^*-1}+1})$. Since the function $$i \mapsto \dfrac{s(q^i+1)+1-2\mathfrak{g}_{a,b}}{q^i-q^{i-1}+1}$$ 
...
\begin{proof}
    ...
    \end{align*}
for any $e \in \set{1,\dots,\lfloor \frac{m}{2} \rfloor}$ (here, we used Lemma \TODO{cond inc $T_i$} to bound the dimension on $\calT_e(s,g)$).
