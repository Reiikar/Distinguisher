...
\begin{def1}
	...
	\[
	\calC_{\mid\mathbb{F}_q}=\calC \cap \mathbb{F}_q^m,
	\]
...
For a vector $c \in \fqm^n$, $\Tr{c}= (\Tr{c_1},\cdots,\Tr{c_n})$. For a linear code $\calC$ of length $n$ and dimension $k$ over $\mathbb{F}_{q^m}$, its image under the trace operator $\Tr{\calC}$ is a linear code of length $n$ over $\mathbb{F}_q$. Its dimension satisfies
...
\begin{thm} \label{Tr_BoundSchurSquare}
		\jade{Ce sera défini avant : With usual definitions of the trace and the $\star$-product of codes,} we have 
		\[ \Tr{C}^{\star2} := ((C^{\perp}_{|_{{\mathbb{F}_q}}})^{\perp})^{\star2} \subseteq \sum\limits_{i=0}^{\lfloor{m/2} \rfloor} \Tr{C\star C^{q^i}},\]
...
\textcolor{blue}{
	\begin{thm} \label{th1}
		...
		\[ \Tr{\calC}^{\star2} := (\mathrm{SSAG}_{q}(\calH,\calP,D')^{\perp})^{\star2} \subseteq \sum\limits_{i=0}^{\lfloor{m/2} \rfloor} \Tr{\mathcal{C}\star\mathcal{C}^{q^i}},\]
...
Now we only have to prove Proposition \ref{powers_of_q's_case} in the case $s=\mu_g+q^k$ for some $0 < k < \frac{m}{2}$. In this case, finding a valid decomposition of each element of $A^s_{i,q}$ in $V^s+q^kV^s$ is intricate. We shall directly proves the equality of spaces 
...
Our goal is to prove that any $(i,j) \in \calA^k$ is in $\calB^k$; or more precisely, in some $\calS^k_{(i_2,j_2)}$. This will show that any element in the basis of $\calL((q^k+1)sP_{\infty})$ is also in the product $\calL(sP_{\infty}) \cdot \calL(sP_{\infty})^{q^k}$, finishing the proof.
...
\[i_{\calA^k} = \max \{ i \ | \ (i,j) \in \calA^k\} \quad , \quad j_{\calA^k} = \max \{ j \ | \ (i,j) \in \calA^k\}\]
and
\[i_{\calS^k} = \max \{ i \ | \ (i,j) \in \calS^k\} \quad , \quad j_{\calS^k} = \max \{ j \ | \ (i,j) \in \calS^k\}.\]
Since $s=q_0(q_0-1)-1+q^k$, we can explicit theses integers. Noticing that
\[ \left\lfloor \dfrac{s}{q_0} \right\rfloor = q_0-1 + \left\lfloor \dfrac{q^k-1}{q_0}\right\rfloor = q_0-1,\]
we deduce that $i_{\calS^k} = \left\lfloor \dfrac{s}{q_0} \right\rfloor = q_0-1$ and $i_{\calA^k} = \left\lfloor \dfrac{s(q^k+1)}{q_0} \right\rfloor = (q_0-1)(q^k+1)$. To determine $j_{\calS^k}$ and $j_{\calA^k}$ we need to compare $\left\lfloor \dfrac{s}{q_0+1}\right\rfloor$ and $\left\lfloor \dfrac{s(q^k+1)}{q_0+1} \right\rfloor$ to $q_0-1$. We get
\begin{align*}
...
\jade{$\uparrow$ En a-t-on besoin ?} \mathieu{je trouvais ça utile pour voir on sont les sauts}
For the shape of the set $\calA^k$ in the plane, let us consider the points \mathieu{$(i_{\calA^k},0)$ and $(x,j_{\calA^k})$ (where $x$ is the maximal abscissa for a point in $\calA^k$, given its ordinate). By definition of $j_{\calA^k}$, all the points in $\calA^k$ are delimited by the two axis, the segment connecting $(0,j_{\calA^k})$ and $(x,j_{\calA^k})$, and a path connecting $(i_{\calA^k},0)$ and $(x,j_{\calA^k})$ (which remains to be found).} From \eqref{desc}, there are two cases to consider :  
\begin{enumerate}
    \item[$\star$] If $2k \geq \dfrac{m}{2}$, then \mathieu{x} is given by $x=i_{\calA^k}-(q_0-1)$, i.e. all the points lying on the segment between $(i_{\calA^k},0)$ and $(x,j_{\calA^k})$ belong to $\calA^k$;
    \item[$\star$] If $2k < \dfrac{m}{2}$, then $x=i_{\calA^k}-q_0$. In this case, \jade{Moi yen a pas comprendre: there is a gap in the sequence of decreasing $x$-values occuring exactly at the ordinate $q^{2k}$.} In particular, both the points $(i_{\calA^k}-(q^{2k}-1),q^{2k-1})$ and $(i_{\calA^k}-(q^{2k}+1),q^{2k})$ are in $\calA^k$, but $(i_{\calA^k}-q^{2k},q^{2k})$ is not. (Note that this is the only gap.)
\end{enumerate}
The set $\calS^k$ consists of all integral points delimited by the two axes and a path connecting the points $(q_0-1,0)$ and $(0,q_0-2)$.  \jade{il faudrait spécifier ce chemin, non ? Pourquoi as a result ? $\rightarrow$} \mathieu{Since $s=i_{\calS^k}q_0+(q^k-1)$ (see \eqref{desc}), there is only one gap in the sequence of decreasing $x$-values, which occurs at the ordinate $q^k$. Thus, $\calS^k$ is delimited by the points $(0,0)$, $(i_{\calS^k},0)$, $(i_{\calS^k}-(q^k-1),q^k-1)$, $(i_{\calS^k}-(q^k+1),q^k)$ and $(0,j_{\calS^k})$.}
You can find the shape of theses sets in Figures \ref{fig_case_2k<m/2} and \ref{fig_case_2k>=m/2} below.
...
At this stage, it remains to prove that every point in $\calA^k$ belongs to some $\calS^k_{(i_2,j_2)}$, for $(i_2,j_2) \in \calS^k$. Note that in the plan, the set $\calS^k_{(i_2,j_2)}$ has the same shape as $\calS^k$, but its bottom left point has coordinates $(i_2,j_2)$ instead of the origin $(0,0)$. We will prove first it the case $2k \geq \frac{m}{2}$, similar arguments works for the other one. Taking a point $b(i_b,j_b)$ in $\calA^k$, we can distinguish two cases:

\begin{itemize}
    \item[$\star$] If $i_b < (q_0-1)q^k$, then $b$ is in the rectangle delimited by \mathieu{the origin $O$, $P(0,q_0-1)$, $Q((q_0-1)q^k-1,q_0-1)$ and $R((q_0-1)q^k-1,0)$.}  %(note that $(q_0-1)q^k=i_{A^k}-(q_0-1)$).
Let us write
    \[i_b = \alpha q^k+\gamma, \quad \mathrm{with} \quad \gamma < q^k.\]
    We want to show that all points $(i_b,0),\cdots,(i_b,q_0-1) \in \calB^k$, to ensure that $b$ is also. In particular, if we denote by $j^*:=\max \{j \ | \ (\alpha,j) \in \calS^k\} = \left\lfloor \frac{s-\alpha q_0}{q_0+1}\right\rfloor$, we will show that all theses points are in 
    \[\bigcup_{j=0}^{j^*} \calS^k_{(\alpha,j)}.\]
    In fact, let $0 \leq j \leq j^*$. Then the set $\calS^k_{(\alpha,j)}$ contains $(i_b,jq^k+r)$ for any $0 \leq r \leq q_0-2-\gamma$ (note that we have no problem with the gap appearing in $\calS^k$ since it occurs at $q^k$, in which case we rather consider the set  $\calS^k_{(\alpha+1,j)}$ instead of  $\calS^k_{(\alpha,j)}$). Since $(j+1)q^k \leq jq^k + (q_0-2-\gamma)+1$ (the equality is attained with $q^k=q=2$ and $q_0=4$), we do not miss any integer from $\calS^k_{(\alpha,j)}$ to $\calS^k_{(\alpha,j+1)}$. It remains to show that $j^*$ is big enough to reach the maximal ordinate this way, which is $q_0-1$. Again, the worst case (minimal value of $j^*$) is attained when $\alpha = q_0-2$ is maximal. In this situation, we have 
    \[j^* = \left\lfloor \dfrac{q_0+q^k-1}{q_0+1}\right\rfloor \geq 1,\]
    and the biggest ordinate reached is $q^k+q_0-2-\gamma \geq q_0-1$, since $\gamma < q^k$. To sum up, we are able to reach all points in the rectangle. In Figure \ref{ex_in_rectangle}, we can see that $A \in \calS^k_{(q_0-4,1)}$. 
...
\node[draw,circle,inner sep=1pt,fill,black,label=270:$R$] at (23,0) {};
...
\end{scope}
	...
	\node[yshift=0.7em] at ($(0,0)!0.5!(E)+({\q*(\qO-4)},0)$) {$\calS^k_{(q_0-4,0)}$};
	\node[yshift=0.5em] at ($(0,0)!0.5!(E)+({\q*(\qO-4)},\q)$) {$\calS^k_{(q_0-4,1)}$};
...
\end{tikzpicture} 
\end{center}
\caption{Example with $2k \geq \frac{m}{2}$ and $i_b \leq (q_0-1)q^k$} \label{ex_in_rectangle}
\end{figure}

 \item[$\star$] Else, we have $q^k(q_0-1) \leq i_b \leq (q^k+1)(q_0-1)$, i.e $b$ is in the triangle delimited by the points \mathieu{$T((q_0-1)q^k,0)$, $U((q_0-1)q^k,q_0-1)$ and $V((q_0-1)(q^k+1),0)$, which we denote $\Delta$ from now on.} We claim the following:
 \[ \forall \delta \in \{1,..,q^k\} \ , \ \calS^k_{(q_0-\delta,\delta-1)} \in \calB^k.\]
 To prove this, it suffices to show that $(q_0-\delta,\delta-1) \in \calS^k$, which is true since
 \[q_0(q_0-\delta) + \delta(q_0+1) = q_0(q_0-1)+\delta-1 \leq s.\]
 Next, we aim to prove
 \begin{equation} \label{counting}
 \Delta \subseteq \bigcup_{\delta=1}^{q^k} \calS^k_{(q_0-\delta,\delta-1)}.
 \end{equation}
 To do that, we start by counting the points in $\Delta$ and $\calS^k_{(q_0-1,0)}$. A short computation gives
 \[ \# \Delta = \dfrac{q_0(q_0+1)}{2} \quad \mathrm{and} \quad \# \calS^k_{((q_0-1)q^k,0)} := \dim \calL(sP_{\infty}) = s+1-g(\calH) = \dfrac{q_0(q_0-1)}{2} + q^k.\]
By definition, $\calS^k_{(q_0-1,0)} \subseteq \Delta$, and since $\# (\Delta \backslash  \calS^k_{((q_0-1)q^k,0)}) = q_0-q^k$, there is only a few points left to care about.
 They are included in the line joining $((q_0-1)(q^k+1),0)$ and $((q_0-1)q^k,q_0-1)$, since all points of cordinates $(i_{\calA^k}-\mu,\mu) (0 \leq \mu \leq q_0-1)$ can not be in $\calS^k_{((q_0-1),0)})$. For these $q_0$ points, we write 
 \[ \mu = \mu_1q^k + \mu_2,\]
 ...
 \[(i_{\calA^k}-\mu,\mu) \in \calS^k_{q_0-1-\mu_1,\mu} \subseteq \bigcup_{\delta=1}^{q^k} \calS^k_{(q_0-\delta,\delta-1)}.\]
Remark that $i_{\calA^k}-\mu = (q_0-1-\mu_1)q^k+(q_0-1-\mu_2)$, so we are led to prove $((q_0-1-\mu_2),\mu_2) \in \calS^k$, which again olds since $\mu_2 < q^k$. Note that $q^k$ of these points are in $\calS^k_{((q_0-1),0)})$ (case $\mu_1=0)$, meaning that the remaining $q_0-q^k$ are in $(\Delta \backslash  \calS^k_{((q_0-1),0)})$, which proves \eqref{counting} (see the Figure \ref{ex_in_triangle} to compare $\Delta$ and $\bigcup_{\delta=1}^{q^k} \calS^k_{(q_0-\delta,\delta-1)}$ on a example).
...
%Points du triangle
\node[draw,circle,inner sep=1pt,fill,black,label=270:$T$] at (24,0) {};
...
\end{tikzpicture} 
\end{center}
\caption{Example with $2k \geq \frac{m}{2}$ and $i_b \geq (q_0-1)q^k$} \label{ex_in_triangle}

\end{figure}
\end{itemize}

We still have to deal with the case $2k < \frac{m}{2}$. Suppose it, then take a point $b(i_b,j_b) \in \calA^k$. Actually, we can proceed the exact same way if $i_b < q^k(q_0-1)$. It implies that we are left with $q^k(q_0-1) \leq i_b \leq (q^k+1)(q_0-1)$, i.e $b$ is in the polygone $\Gamma$ delimited by the points \mathieu{$A(q^k(q_0-1),0)$, $B(q^k(q_0-1),q_0-2)$, $C(i_{A^k}-(q^{2k}+1),q^{2k})$, $D(i_{A^k}-(q^{2k}-1),q^{2k}-1)$ and $E(i_{A^k}$,0).} As above, we have:
 \[ \forall \delta \in \{1,..,q^k\} \ , \ \calS^k_{(q_0-\delta,\delta-1)} \in \calB^k.\]
 and 
 \[\calS^k_{(q_0-1,0)} \subseteq \Gamma.\]
This time, the same comutation trick gives
\[ \# \Gamma = \dfrac{q_0(q_0-1)}{2}+q^{2k} \quad \mathrm{and} \quad \# \calS^k_{(q_0-1,0)} = \dfrac{q_0(q_0-1)}{2}+q^{k}.\]
It implies $\# (\Gamma \backslash  \calS^k_{(q_0-1,0)}) = q^{2k}-q^k$. Again, those points are in the path joining ($i_{\calA^k}$,0) and  $(q^k(q_0-1),q_0-2)$, which contains $q_0-2$ points in total. Amoung them, some are already in $\calS^k_{(q_0-1,0)}$, exepted for (this is due to the gap at $q^{2k}$) 
\[ (i_{\calA^k}-\mu,\mu) \ , \ \mathrm{where} \ \mu \in \{q^k,\cdots,q^{2k}-1\}.\]
For such points, we write
\[\mu = \mu_1 q^k + \mu_2 \ , \ \mathrm{with} \ \mu_1,\mu_2 < q^k,\]
and we claim that $(i_{\calA^k}-\mu,\mu) \in \calS^k_{((q_0-1)-\mu_1,\mu_1)} \in \calB^k$, which holds since both $\mu_1$ and $\mu_2$ are smaller than $q^k$. \mathieu{Figure \ref{ex_polygone} below show an example in this case.}
...
%Points du polygone
\node[draw,circle,inner sep=1pt,fill,black,label=270:$A$] at (14,0) {};
\node[draw,circle,inner sep=1pt,fill,black,label=$B$] at (14,6) {};
\node[draw,circle,inner sep=1pt,fill,black,label=$C$] at (16,4) {};
\node[draw,circle,inner sep=1pt,fill,black,label=$D$] at (18,3) {};
\node[draw,circle,inner sep=1pt,fill,black,label=$E$] at (21,0) {};
...
\end{tikzpicture} 
\caption{Example with $2k < \frac{m}{2}$ and $i_b \geq q^k(q_0-1)$} \label{ex_polygone}
\end{center}
\end{figure}
