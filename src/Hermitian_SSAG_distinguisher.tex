...
MCEliece cryptosystem is the only code-based candidate for standartisation of post--quantum cryptrographic to the NIST competition since the $3^rd$ round. This motivates the study of structural attacks and of the possibilities of parameters improvements. 
...
\TODO{Citer quelques tentatives}
...
\TODO{Parler des Goppa--like AG codes}
...
As Goppa--like AG codes naturally generalizes Goppa codes, it is natural to wonder if one can derive a structural attack on these codes from Mora and Tillich's attack. The genus of the curve plays a significant role in the parameters of AG codes. The greater the genus, the more $\Fq$--points the curve \calX$ may have and so the longer the code can be. But the parameters $[n,k,d]$ of an AG code satisfies $n-g+1 \leq k+d \leq n+1$, which means that an AG code is $g$--far form optimality. Also, the correction capacity naturally suffers from a big genus: the naive correction algorithm can correct up to $\frac{d-1-g}{2}$ errors. Only refined techniques, based on error locating pairs, manage to remove this drawback. Caution should be exercised when it comes to the impact of the genus on the properties of AG codes. In the present work, we interest in the behaviour of Goppa--like one--point AG codes from $\calC_{a,b}$ curves with respect to a similar structural attack.

AG codes have a very similar behaviour as GRS codes with respect to Schur product. Moreover, some well-chosen AG codes are defined by the evaluation of \textit{multivariate} polynomials. In this case, we prove that performing division algorithm via Gr\¨obner bases enable us to estimate the dimension of the square of the dual of Goppa--like AG codes.
...
\noindent These curves are well-studied, and we know that any curve $\calX_{a,b}$ defined by an equation as in \eqref{eq:equation_C_ab} as a unique point at infinity, denoted by $P_{\infty}$. Moreover, its genus is also known:
 $$g\mathfrak{g}(\calX_{a,b}) := \mathfrak{g}_{a,b} = \dfrac{(a-1)(b-1)}{2}.$$
...
\paragraph{One-point codes.} We will consider codes obtained by evaluating functions on $\calX_{a,b}$ which are regular everywhere, except maybe at the unique point at infinity $P_\infty$ on $\calX_{a,b}$. These functions then belong to the ring 

\[\calO_{P_\infty}=\bigcup_{s \geq 0} \calL(s P_\infty)\]
...
\TODO{Garde-t-on ceci ? Let $\calP \subseteq \fqm(\calX_{a,b}) \backslash \set{P_{\infty}}$ be a set of length $n$ and $s > 0$. We define the one-point AG code associated to the support $\calP$ and the integer $s$ by
$$\calC_{\calL}(\calX_{a,b},\calP,sP_{\infty}).$$}

\jade{Pour moi, le Goppa--like AG code devrait être le SSAG de ça. Le truc dessous serait plutôt un generalized AG code, pour lequel le diviseur n'est pas effectif...}
...
\noindent The terminology Goppa-like is motivated by the fact that $\calC_{\calL}(\calX_{a,b},\calP,sP_\infty+(g))$ is an evaluation code where the function $g$ plays the role of the Goppa polynomial.
...
\noindent Let $s \geq 2\mathfrak{g}_{a,b} -1$ and $\calP \in \calX_{a,b}(\fqm) \backslash \set{P_\infty}$ be a set of length $n$. We consider the one-point code
...
\begin{proof}
    ...
    where $\calC^{q^i+1} = \calC_{\calL}(\calX_{a,b},\calP,s(q^i+1)P_\infty).$ Since $s\geq 2\mathfrak{g}_{a,b}-1$, the Riemann-Roch theorem gives 
    $$\dim_{\fqm} \calC^{q^i+1} = s(q^i+1)+1-\mathfrak{g}_{a,b}.$$
    Finally, \eqref{eq:dim_trace} gives 
    $$\dim_{\fq} \Tr{\calC^{q^i+1}} \leq m(s(q^i+1)+1-\mathfrak{g}_{a,b}).$$
\end{proof}
...
\begin{proposition} \label{prop:bound_dim_using_inclusions}
    Let $\calC := \calC_{\calL}(\calX_{a,b},\calP,sP_{\infty})$ be a one-point AG code on $\calX_{a,b}$ such that $s \geq 2g_{a-b}-1$. Set $k := \dim_{\fqm}\calC = s+1-\mathfrak{g}_{a,b}$ and  $e := \left\lfloor \log_q\left(\dfrac{k^2}{s}\right)\right\rfloor$. Then
...
\begin{proof}
    ...
    \begin{align*}
        ...
        & \leq \sum\limits_{i=0}^{e} m\dim_{\fq} \calC^{q^i+1} + \sum\limits_{i=e+1}^{\lfloor m/2 \rfloor} \Tr{\calC \star \calC^{q^i}} \\
...
\TODO{include dimension $R(g^{q^i
-q^{i-1}+1})$?}	
...
In the proposition below, we compute the exact dimension of $\calT_i$'s.

\begin{proposition} \label{prop:dim_M_i's}
Fix $i \in \set{1,\dots,\lfloor m/2 \rfloor}$ and
    let $\alpha' \in \set{0,\dots,a-1}$ be the rest of the Euclidean division of $\alpha (q^i-q^{-1}+1)$ by $a$. Then we can compute the exact dimension of $M_i := R(g^{q^{i}-q^{i-1}+1}) \cap \calL(s(q^i+1)P_\infty)$ depending on $s'$:
    \begin{enumerate}
...
Note that case $1)$ is not really interesting since it correspond to the case where $\calT_i = \Tr{\calL(s(q^i+1)P_\infty)}, $ meaning that considering the $T_i's$ will produce the same bound for $\Tr{\calC}^{\star 2}$ as the one given in \ref{prop:bound_dim_using_inclusions}.
In what follows, we will focus on case $2)$, since under the corresponding conditions we can prove that $(\calT_i)_i$ forms an increasing sequence for the inclusion. We start with an easy lemma:
...
$$\dim_{\fq} \Tr{\calC}^{\star 2} \leq \binom{mk+1}{2} - \dfrac{m}{2}(k^2(2e^*+1)+k-2s'(q^{e^*}-q^{e^*-1}+1)). $$
\end{coro}
...
\section{Secured parameters against the distinguisher}

\noindent Given a Goppa like code $\calC := C_\calL(\calX_{a,b},\calP,D)$, where $D = sP_\infty +(g)$ and $\degab{g} = s'>s\geq 2\mathfrak{g}_{a,b}-1$, we proved above that whenever $$s \geq (s'-s)q+2\mathfrak{g}_{a,b}-1,$$ we have 

\begin{equation} \label{eq:best_upper_bound}
\dim_{\fq} \Tr{\calC}^{\star2} \leq \frac{m}{2}\left(2s'(q^{e^*}-q^{e^*-1}+1)+k^2(m-1-2e^*)  \right),
\end{equation}
where $e^* := \left\lceil \log_q\left(\dfrac{k^2}{s'(q-1)^2}\right)\right\rceil+1$.
From the definition of $\calC$, we can estimate its length $n$. In fact, since $\calP \cap \Supp(g) = \varnothing,$ the maximal possible length of $\calC$ is reached when $\calP = \calX_{a,b}(\fqm) \backslash \Supp(g)$ and $g$ has only one zero, that is
$$n = \# \calP = |\calX_{a,b}(\fqm)| \leq q^m-1+2\mathfrak{g}_{a,b}\sqrt{q^m},$$
using the Hasse-Weil bound. We can now give the choices of $s$ and $s'$, depending on the fixed parameters $a,b,q$ and $m$, for which
$$\frac{m}{2}\left(2s'(q^{e^*}-q^{e^*-1}+1)+k^2(m-1-2e^*)  \right)\geq q^m-1+2\mathfrak{g}_{a,b}\sqrt{q^m},$$
in which cases the corresponding Goppa like code resist the distinguisher.


\TODO{Je galère un peu à établir des conditions "claires" sur $s$ et $s'$ en fonction de $a,b,q$ et $m$... Je propose pour le moment de faire des tests avec des codes qui me semblent pertinent (elliptique, hermitien,...); en donnant les conditions en question avec Magma. À voir ce qu'on en fait après.}
