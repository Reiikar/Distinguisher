...
\begin{def1}[Linear code]
  A linear code $C$ is a subspace of $\mathbb{F}_{q}^n$. \jade{The integer $n$ is called its length and we denote by $k$ its dimension, and $d$ its the minimum distance. We say that $C$ is a $[n,k,d]_q$ code or that it has parameters $[n,k,d]_q$.}
\end{def1}
...
\begin{def1}[Dual code]
	Let $C$ be a linear code of length $n$. The dual code $C^{\perp}$ of $C$ is \[
	C^{\perp}=\left\lbrace x \in \mathbb{F}_{q}^n \mid c \cdot x=0, \text{ for all } c \in C \right\rbrace,
	\]  
	\jade{where $\cdot$ denotes the usual scalar product.}
\end{def1}

\jade{If the code $C$ has parameters $\left[n,k \right]$, then its dual $C^{\perp}$ is an $[n,n-k]$ linear code.}
...
\begin{def1}
	Let $C$ be a $[n,k]$ linear code over $\mathbb{F}_{q^m}$. The subfield subcode $C_{\mid\mathbb{F}_{q}}$ of $C$ is defined by
	\[
	C_{\mid\mathbb{F}_q}=C \cap \mathbb{F}_q^m,
	\]
...
\jade{Il faut qu'on se mette d'accord sur la notation Tr !}
...
\TODO{Delsarte}
	...
	\begin{lem1} \label{known_bounds}
		...
		\begin{itemize}
			\item[$(1)$] $\dim_{\mathbb{F}_q}(\Tr(\calC)) \leq \min\{mk_{\calC},n\}$; \TODO{Séparer les résultats Tr/SS et $\star$-product}
