...
$$ E : y^2 = x^3+2x+1.$$
As usual, we denote by $P_\infty$ the unique point at infinity on $E$. Wa take $D$ as a one-point divisr supported by $P_\infty$, \emph{i.e} $D = sP_\infty$. For a fixed degree $s=7$, we consider two choices of function $g$:
\begin{enumerate}
...
Consequently, we easily check that $\deg\left(\left[\frac{G}{q} \right]\right) = -1 \leq 2g(E)-2$. Some computations realized on Magma show that the codes $\Gamma(\calP,D,g_1)$ and $\mathrm{Car}_3(\calP,G)$ are not equal in this case.
\end{enumerate}
