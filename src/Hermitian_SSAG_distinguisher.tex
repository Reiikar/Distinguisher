...
\end{equation}
 ...
 $$\left\lfloor \frac{G}{q} \right\rfloor := \sum\limits_{\nu_P(G) > 0} \left\lfloor \frac{\nu_P(G)}{q} \right\rfloor P+ \sum\limits_{\nu_P(G)< 0}\nu_P(G)P.  $$
Thus, whenever $\deg\left\lfloor \frac{G}{q} \right\rfloor > 2\mathfrak{g}-2$, the corresponding Cartier code coincides with the Goppa--like AG code. \mathieu{Doit-on préciser la condition plus tard pour les one--point ? En disant que cela impose des conditions sur $(g)_0$}.
