...
In this work, we also investigate Goppa--like AG codes over finite fields with different characteristics by analysing the computational cost of the ISD algorithm for parameters relevant to post--quantum cryptography. In Table \ref{table:goppa-herm}, we provide parameters that resist the distinguisher given in this paper and improve key sizes compared to the subfield subcodes of one--point Hermitian codes parameters reported in \cite[Tables~2 and 3]{EKN21crypto}, which already reduced key sizes compared to binary Goppa codes.
...
\begin{table}[h]
	\begin{center}
			\begin{tabular}{|c|c|c|c|c|c|}
				\hline
				Security level&  $n$ & $k$ & $t=r$ & Security bits& Key--Size(Bytes)\\
				\hline \hline
				Category 1 	&	3\,488 &2\,720 &64  &143 &261\,120 \\\hline
				Category 3 	&	4\,608 &3\,360 &96  &185 &524\,160 \\\hline
				Category 5 	&	6\,688 &5\,024 &128 &263 &1\,044\,992 \\
...
$$ E : y^2 = x^3+2x+1.$$
As usual, we denote by $P_\infty$ the unique point at infinity on $E$. We take $D$ as a one--point divisor supported by $P_\infty$, \emph{i.e} $D = sP_\infty$. For a fixed degree $s=7$, we consider two choices of function $g$, each time such that $g \in \calL(8P_\infty)$:
\begin{enumerate}
\item First, let $g_1 = x^4 \notin \calL(D)$. In this case, $g$ has 2 zeros (say $P_1$ and $P_2$), which are those of $x$. More precisely, we have
...
\item Second, take $g_2 = x^4 + xy$. This time, we can verify that $g_2$ has $8$ distincts rational zeros, say $R_1,\dots, R_8$, \emph{i.e}
...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|}
			\hline
			$n$&  $s = \deg(D)$ & Choice of $g$ & $\deg\left(\left[\frac{G}{q} \right]\right)$  & $\dim_{\mathbb{F}_3}(\Gamma(\calP,D,g))$ &$\dim_{\mathbb{F}_3}(\mathrm{Car}_3(\calP,G))$ \\
			\hline \hline
			88 & 7 & $g = x^4$ & 1 & 64 & 64 \\
			\hline
...
\jade{On devrait dire qu'on retrouve \cite{FGO+13} pour $\calX=\PP^1$.}
...
\mathieu{Ma proposition d'exemple $\downarrow$}

\begin{example} \label{ex:non_equality_bound}
In \cite{FGO+13}, the authors predict that for "generic" alternant codes (\emph{i.e} Goppa--like AG codes constructed on the projective line), the bound given in Proposition \ref{prop:bound_dim_using_inclusions} is sharp. A consequence of Proposition \ref{prop:non-eq} is that a way to be "non--generic" is to have $\calC_1 \neq \{0\}$. We now illustrate this in a toy example. \\
Let $q=5$ and $m=4$. Let $P_\infty$ be the point at infinity on the projective line $\PP^1$ over $\fqm$. Using Magma, we compare the bound given in Proposition \ref{prop:bound_dim_using_inclusions} with the true dimension of the corresponding code. In Table \ref{table:example2} bellow, we take $\calP = \PP^1(\fqm) \setminus \Supp(g)$, $D = sP_\infty$ and $P_0$ stands for the zero of $x$ in the rational function field $\fqm(x)$.
\begin{table}[h]
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|c|}
			\hline
			$n$&  $s = \deg(D)$ & Choice of $g$ & $\left[ \frac{D+(g)}{q} \right]$ & $\dim_{\fqm} \calC_1$  & $\dim_{\fq} (\Gamma(\calP,D,g)^{\perp})^{\star 2}$ & Bound in Prop \ref{prop:non-eq}\\
			\hline \hline
			624 & 9 & $g = x^{10}$ & $2P_0-P_\infty$ & 2 & 255 & 496\\
			\hline
			624 & 10 & $g = x^{11}$ & $2P_0-P_\infty$ & 2 & 295 & 570\\ 
			\hline
		\end{tabular}
		\vspace*{0.3em}
		\caption{Illustration of Proposition \ref{prop:non-eq}} \label{table:example2}
	\end{center}
\end{table}
\end{example}
...
\begin{remark} \label{rq:classical_Goppa_case}
