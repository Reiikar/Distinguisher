...
\calL(sP_{\infty}) = \langle x^iy^j : 0 \leq j \leq i_{\calA^k} \ \mathrm{and} \ iq_0+j(q_0+1) \leq s \rangle_{\mathbb{F}_{q_0^2}}
\end{equation}
...
The set $\calS^k$ consists of all integral points of the non--convex polygon of vertices $O$, $(i_{\calS^k},0)$, $(i_{\calS^k}-(q^k-1),q^k-1)$, $(i_{\calS^k}-(q^k+1),q^k)$ and $(0,j_{\calS^k})$.

Regarding the shape of the set $\calA^k$ in the plane, two cases can be distinguished.
\begin{itemize}
	\item if $2k \geq \frac{m}{2}$, $\calA^k= \Gamma \sqcup \Delta$ where $\Gamma$ is the set of integral points in the rectangle of vertices $O$, $P(0,j_{\calA^k})$, $Q(i_{\calS^k}q^k-1,j_{\calA^k})$ and $R(i_{\calS^k}q^k-1,0)$ and $\Delta$ those in the triangle of vertices $T(i_{\calS^k}q^k,0)$, $U(i_{\calS^k}q^k,j_{\calA^k})$ and $V(i_{\calA^k},0)$. 
	\item if $2k < \frac{m}{2}$, $\calA^k= \Gamma' \sqcup \Delta'$ where $\Gamma$ is the set of integral points in the rectangle of vertices $O$, $P$, $Q'(i_{\calS^k}q^k,j_{\calA^k})$ and $R'(i_{\calS^k}q^k,0)$ and $\Delta'$ those in the non--convex polygon of \modifier{vertices... $T(i_{\calA^k}q^k,0)$, $U(i_{\calA^k}q^k,j_{\calA^k})$ and $V(i_{\calA^k}(q^k+1),0)$}. 
\end{itemize}

You can find the shape of theses sets in \modifier{Figures \ref{fig_case_2k<m/2} and \ref{fig_case_2k>=m/2} }below.\textbf{}

\TODO{Mettre ces points sur les figures dessous et dessiner le rectangle/triangle en pointillés}

\vspace*{0.3cm}
\begin{figure}[h] 
	\centering
	\input{fig_2k_sup_m_sur_ 2}
	\caption{Case $2k \geq \frac{m}{2}$} \label{fig_case_2k>=m/2}
\end{figure}
\begin{figure}[h] 
	\centering
	\input{fig_2k_inf_m_sur_ 2}
	\caption{Case $2k < \frac{m}{2}$} \label{fig_case_2k<m/2}
\end{figure}

 To prove that $\calA^k$ is included in $\calB^k$, we will use its partition into $\Gamma$ and $\Delta$ (or $\Gamma'$ and $\Delta'$).

% let us consider the points \mathieu{$(i_{\calA^k},0)$ and $(x,j_{\calA^k})$ (where $x$ is the maximal abscissa for a point in $\calA^k$, given its ordinate). By definition of $j_{\calA^k}$, all the points in $\calA^k$ are delimited by the two axes, the segment connecting $(0,j_{\calA^k})$ and $(x,j_{\calA^k})$, and a path connecting $(i_{\calA^k},0)$ and $(x,j_{\calA^k})$ (which remains to be found).} From \eqref{desc}, there are two cases to consider :  
%\begin{enumerate}
...
%The set $\calS^k$ consists of all integral points delimited by the two axes and a path connecting the points $(q_0-1,0)$ and $(0,q_0-2)$.  \jade{il faudrait spécifier ce chemin, non ? Pourquoi as a result ? $\rightarrow$} \mathieu{Since $s=i_{\calS^k}q_0+(q^k-1)$ (see \eqref{eq:s_iAk_jAk}), there is only one gap in the sequence of decreasing $x$-values, which occurs at the ordinate $q^k$.}



%At this stage, it remains to prove that every point in $\calA^k$ belongs to some $\calS^k_{(i_2,j_2)}$, for $(i_2,j_2) \in \calS^k$.

%Note that in the plan, the set $\calS^k_{(i_2,j_2)}$ has the same shape as $\calS^k$, but its bottom left point has coordinates $(i_2,j_2)$ instead of the origin $(0,0)$.

Let us first focus on the case $2k \geq \frac{m}{2}$.
...
\item To prove that $\Gamma \subset \calB^k$, we will fix an abscissa $i$ of a point in $\Gamma$ and we will show that all points $(i,0),\cdots,(i,q_0-1)$ belong to $\calB^k$. By definition of $\Gamma$, we have $i < i_{\calS^k}q^k=(q_0-1)q^k$. Let us write
    \[i = \alpha q^k+\gamma \text{ with } \alpha <q_0 \text{ and } \gamma < q^k.\]
   
   	Set $j^*:=\max \{j \ | \ (\alpha,j) \in \calS^k\} = \left\lfloor \frac{s-\alpha q_0}{q_0+1}\right\rfloor$, we will show that  
    ...
    \[\left\{ (i,0),\cdots,(i,j^*q^k+q_0-2-\gamma) \right\} \subset \bigcup_{j=0}^{j^*} \calS^k_{(\alpha,j)}.\]    
	One can easily check that    
    \begin{align*}
    	\min \{y \mid (i,y) \in \calS^k_{(\alpha,j+1)} \} & =     (j+1)q^k, \\
    	\max \{y \mid (i,y) \in \calS^k_{(\alpha,j)}\} & = jq^k + (q_0-2-\gamma)
    \end{align*}
	and we have the desired inequality, reached for $q^k=q=2$ and $q_0=4$. Now, it remains to prove that $j^*q^k+q_0-2-\gamma \geq q_0-1$. The minimal value of $j^*$ is attained when $\alpha = q_0-2$ is maximal. In this situation, we have 
    \[j^* = \left\lfloor \dfrac{q_0+q^k-1}{q_0+1}\right\rfloor \geq 1,\]
    and $q^k+q_0-2-\gamma \geq q_0-1$, since $\gamma < q^k$. %To sum up, we are able to reach all points in the rectangle. In Figure \ref{ex_in_rectangle}, we can see that $A \in \calS^k_{(q_0-4,1)}$. 
...
Therefore, to prove that $\calS^k_{(i_{\calS^k},0)} \subset \Delta$, it is enough to check that $\calS^k_{(i_{\calS^k},0)} \subset \calA^k$. For any $(i_0,j_0) \in \calS^k$, we have
\[(i_{\calS^k}q^k+i_0)q_0+j_0(q_0+1)\leq i_{\calS^k}q^kq_0+s \leq s(q^k+1).\]
...
$\Delta \setminus  \calS^k_{(i_{\calS^k},0)}$ lies in some $\calS^k_{(i_{\calS^k}-\delta,\delta)}$ for $\delta\geq 1$.
 ...
 To determine $\Delta \setminus  \calS^k_{(i_{\calS^k},0)}$, we start by counting the points in both sets. A short computation gives
 \[ \# \Delta = \dfrac{q_0(q_0+1)}{2} \quad \mathrm{and} \quad \# \calS^k_{(i_{\calS^k},0)} = \dim \calL(sP_{\infty}) = s+1-g(\calH) = \dfrac{q_0(q_0-1)}{2} + q^k.\]

Then $\# \Delta \setminus  \calS^k_{(i_{\calS^k},0)}=q_0-q^k$. Besides one can easily check that the $q_0-q^k$ points of coordinates $(i_{\calA^k}-\mu,\mu)$ for $q^k \leq \mu \leq q_0-1$ cannot be in $\calS^k_{(i_{\calS^k},0)}$.

Then $\Delta \setminus  \calS^k_{(i_{\calS^k},0)}$ consists precisely in these points. For these points, we write 
 \[ \mu = \mu_1q^k + \mu_2,\]
 with $\mu_2 < q^k$ and $1 \leq \mu_1 \leq \left\lfloor\frac{q_0-1}{q^k}\right\rfloor \leq q^{m/2-k}-1 \leq q^k-1$ (recall $k \geq m/4$). Finally we claim
...
\modifier{(see the Figure \ref{ex_in_triangle} to compare $\Delta$ and $\bigcup_{\delta=1}^{q^k} \calS^k_{(q_0-\delta,\delta-1)}$ on a example).}
...
To determine $\Delta \setminus  \calS^k_{(i_{\calS^k},0)}$, we compute 
\[ \# \Gamma = \dfrac{q_0(q_0-1)}{2}+q^{2k} \quad \mathrm{and} \quad \# \calS^k_{(q_0-1,0)} = \dfrac{q_0(q_0-1)}{2}+q^{k},\]
which gives $\# (\Gamma \backslash  \calS^k_{(q_0-1,0)}) = q^{2k}-q^k$. Again, those points are in the path \modifier{joining ($i_{\calA^k}$,0) and  $(q^k(q_0-1),q_0-2)$}, which contains $q_0-2$ points in total. Amoung them, some are already in $\calS^k_{(q_0-1,0)}$, excepted for (this is due to the gap at $q^{2k}$) 
