...
with $c \neq 0$, $\alpha \leq a-1$ and $f' \in \calS$ such that any monomial $x^iy^j$ of $f'$ satisfies $ai+bj < \degab{x^{\beta}y^{\alpha}}$ and $j \leq a-1$. The leading monomial of $f$ with respect to the monomial order $\prec$ is thus defined by \mathieu{$\mathrm{LM}(f) := c \cdot x^{\beta}y^{\alpha}$}. This extends the definition of weighted degree to any such function by setting 
\[\degab{f} := \degab{\mathrm{LM}(f)}.\]
\end{definition}
\mathieu{Note that if $c \neq 1$, we set $\degab{c \cdot x^{\beta}y^{\alpha}} = \degab{x^{\beta}y^{\alpha}}$. Up to a division by $c \neq 0$, we from now on always assume that $c=1$.}
...
\begin{example} We take the same settings as in Example \ref{ex:Goppa_vs_Cartier}. Taking $g_1 = x^4$, we saw that $\deg\left(\left[\frac{G}{q}\right]\right) = 1$. Using Magma, we can show that the dimension of the code $\calC_1$ defined in \ref{prop:non-eq} equals 1. \mathieu{bon, il faudrait trouvé autre chose parce la, notre code est plein de dim 88 et la borne vaut 266, donc bien sur elle est pas atteinte... Concrètement, avec ce genre de paramètres, notre borne dépasse la longueur à partir de s=3, et je ne peux pas avoir de $C_1$ non trivial pour de si petit $s$}
\end{example}
...
\jade{J'ai retravaillé la partie ci-dessous...}

\subsection{Study of the codes $\Tr{\calC \star \calC^{q^i}}$}
