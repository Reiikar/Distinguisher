...
\begin{table}[h]
	\begin{center}
			\begin{tabular}{|c|c|c|c|c|c|}
				...
				\hline \hline
				Category 1 	&	3\,488 &2\,720 &64  &143 &2\,088\,960 \\\hline
		...
		\end{tabular}
		\vspace*{0.3em}
		\caption{McEliece cryptosystem based on \textbf{binary Goppa codes} \jade{citer spécifications}}
	\end{center}
\end{table}
...
$$ E : y^2 = x^3+2x+1.$$
As usual, we denote by $P_\infty$ the unique point at infinity on $E$. We take $D$ as a one--point divisor supported by $P_\infty$, \emph{i.e} $D = sP_\infty$. For a fixed degree $s=7$, we consider two choices of function $g$:
\begin{enumerate}
...
\item Second, take $g_2 = x^4 + xy$. Again, we have $g_2 \notin \calL(D)$ since it has same valuation at $P_\infty$ as $g_1$. This time, we can verify that $g_2$ has $8$ distincts rational zeros, say $R_1,\dots, R_8$, \emph{i.e}
...
\end{enumerate}
Magma results are summarized in Table \ref{table:example1}.
\begin{table}[h]
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|}
			\hline
...
\jade{On devrait dire qu'on retrouve FGO (à ajouter la bib) pour $\calX=\PP^1$.}
...
\begin{example}
	\jade{FOG prévoit égalité pour un alternant générique. Une façon d'être non génrique, c'est d'avoir $\calC_1 \neq \{0\}$.}
\end{example}
