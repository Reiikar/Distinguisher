...
In this section, we briefly introduce some notation and basic definitions for linear codes, subfield subcodes, and trace codes. Furthermore, we present significant results that employ component-wise product and trace map. Let $q$ be a prime power and $\mathbb{F}_q$ be a finite field with $q$ elements. $\mathbb{F}_q^m$ is a finite field extension of $\mathbb{F}_q$ where $m$ is a positive integer. Throughout this paper, we will focus on linear codes defined over $\mathbb{F}_q^m$. 

\begin{def1}[Linear code]
  A linear code $C$ is a subspace of $\mathbb{F}_{q^m}^n$, which is denoted by $\left[n,k,d \right] $ (or $\left[n,k \right] $), where $k$ is the dimension, $n$ is the length, and $d$ is the minimum distance.
\end{def1}

\begin{def1}[Dual code]
	Let $C$ be a linear code $\left[n,k \right] $. The dual code $C^{\perp}$ of $C$ is \[
	C^{\perp}=\left\lbrace x \in \mathbb{F}_{q^m}^n \mid c.x=0, \text{ for all } c \in C \right\rbrace.
	\]  
...
\begin{def1}
	Let $C$ be a $[n,k]$ linear code over $\mathbb{F}_{q^m}$. The subfield subcode $C\mid_{\mathbb{F}_{q^m}}$ of $C$ is by definition:
	\[
	C\mid_{\mathbb{F}_q}=C \cap \mathbb{F}_q^m,
	\]
	
	the set of all codewords in $C$ with components in $\mathbb{F}_q$.
\end{def1}


\TODO{some sentences needed...}

Let $Tr_{\mathbb{F}_{q^m} / \mathbb{F}_q}$ be the trace polynomial in the field $\mathbb{F}_{q^m}$ with respect to $\mathbb{F}_q$, that is 

\[
Tr_{\mathbb{F}_{q^m} / \mathbb{F}_q}(x) = x + x^q + ... + x^{q^{m-1}}.
\]


For a vector $c \in \mathbb{F}_{q^m}^n$, $Tr_{\mathbb{F}_{q^m} / \mathbb{F}_q}(c)= (Tr_{\mathbb{F}_{q^m} / \mathbb{F}_q}(c_1),\cdots,Tr_{\mathbb{F}_{q^m} / \mathbb{F}_q}(c_n))$. For a linear code $C$ of length $n$ and dimension $k$ over $\mathbb{F}_{q^m}$, $Tr_{\mathbb{F}_{q^m} / \mathbb{F}_q}(C)$ is a linear code with the same length of $C$ and dimension $\dim(Tr_{\mathbb{F}_{q^m} / \mathbb{F}_q}(C))? $ over $\mathbb{F}_q$. (A continuer)
