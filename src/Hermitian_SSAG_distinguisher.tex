...
The structure of the AG code $\calC$ may provide sharper bounds on the dimension and the minimum distance of the associated Goppa--like AG code.
...
\end{equation}
Moreover, set $U:=\{P \in \Supp(G) \mid \nu_P(G) \geq 0 \text{ and } \nu_P(G) = q-1 \mod q\}$ and $G_U=\sum{P \in U} P$. Then

\[\dim_{\fq} \Gamma(\calP,D,g)  \geq n-m(\ell(D) - \ell(G1)).\]

\end{theo}

\begin{proof}
{\cite[Theorem~9.1.6]{Sti09}}
\end{proof}
...
\begin{equation} \label{\ref{eq:divisor_G_1}}
G \geq qG_1 \ \mathrm{and} \ G \geq G_1.
\end{equation}
