...
Our starting point is the paper by Mora and Tillich \cite{MT21}, which benefited from the trace structure of the dual code of a subfield subcode to provide a sharp bound for the dimension of the square of the dual of alternant codes. This gives a distinguisher for these codes, as this bound is significantly smaller than expected. \jade{car c'est facile de calculer cette dimension.} Informally they give a bound of the form
\[\dim \left(\left(\GRS_r(\mathbf{x},\mathbf{y})|_{\fq}\right)^\perp\right) ^{\star 2} \leq D  - \delta\]
where $D=\dim \left(\left(C_{|\fq}\right)^\perp\right) ^{\star 2}=\binom{mr+1}{2}$ for a random $r$-dimensional code $C$ and $\delta>0$ (see \S \ref{subsec:def} for formal definitions).
...
\end{equation}
As a consequence, for associated AG codes, we have  $C_{\calL}(\calX,\mathcal{P},G)$
\[ C_{\calL}(\calX,\mathcal{P},F) \star C_{\calL}(\calX,\mathcal{P},G) \subseteq C_{\calL}(\calX,\calP,F+G).\]
In particular, for the Schur square, we have
...
\end{equation}
which is much smaller than the expected dimension given in Lemma \ref{lem:known_bounds} (\ref{it:dim_C^2}). \jade{(thus providing a distinguisher for AG codes). }Combined with Equation \eqref{eq:key_equation}, we get a first upper bound on the dimension of the square of the trace of an AG code.
...
\jade{La borne est vraiment pas optimale... BLABLA}
...
\begin{example}
	Codes studied in Example \ref{ex:non_equality_bound} ...
	\jade{Nouvelles bornes : 400 et 462}
