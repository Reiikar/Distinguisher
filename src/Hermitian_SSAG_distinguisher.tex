...
These codes are interesting because we know an explicit basis of the Riemann-Roch space $\calL(P_{\infty})$, \emph{i.e.}
...
\begin{proposition} \label{prop:bound_dim_using_inclusions}
    Let $\calC := \calC_{\calL}(\calX_{a,b},\calP,sP_{\infty})$ be a one-point AG code on $\calX_{a,b}$ such that $s \geq 2g_{a-b}-1$. Set $k := \dim_{\fqm}\calC = s+1-g_{a,b}$ and  $e := \left\lfloor \log_q\left(\dfrac{k^2}{s}\right)\right\rfloor$. Then
...
Fix $i \in \set{1,\dots,\lfloor m/2 \rfloor}$ and
    ...
    \begin{enumerate}
        \item $\dim_{\fqm}(M_i) = s(q^i+1)+1-g_{a,b}$ if $s' > \left\lfloor \dfrac{s(q^i+1)+a}{q^i+q^{i-1}-1}\right\rfloor$;
        
        \item $\dim_{\fqm}(M_i) = s'(q^i-q^{i-1}+1)$ if $s' \leq \left\lfloor \dfrac{s(q^i+1)+1-2g_{a,b}}{q^i+q^{i-1}-1}\right\rfloor$;
...
\begin{lemma} \label{lem:cond_inclusion_T_i's}
    If $s \geq (s'-s)q+2g_{a,b}-1$, then for every $i \in \set{1,\dots,\lfloor \frac{m}{2} \rfloor}$,
...
\begin{proof}
    ...
    $$M_i = \calR(g^{q^i-q^{i-1}+1}) \iff  s' \leq \left\lfloor \dfrac{s(q^i+1)+1-2g_{a,b}}{q^i+q^{i-1}-1}\right\rfloor.$$
    ...
    $$s' \leq \left\lfloor \dfrac{s(q+1)+1-2g_{a,b}}{q}\right\rfloor;$$
    which gives $s \geq (s'-s)q+2g_{a,b}-1$. The result follows.
\end{proof}

\begin{proposition} \label{prop:inclusion_T_i's} Suppose $s \geq (s'-s)q+2g_{a,b}-1$. Then
...
Suppose $s \geq (s'-s)q+2g_{a,b}-1$ and let $e^* := \left\lceil \log_q\left(\dfrac{k^2}{s'(q-1)^2}\right)\right\rceil+1$. Then
...
\noindent Given a Goppa like code $\calC := C_\calL(\calX_{a,b},\calP,D)$, where $D = sP_\infty +(g)$ and $\degab{g} = s'>s\geq 2g_{a,b}-1$, we proved above that whenever $$s \geq (s'-s)q+2g_{a,b}-1,$$ we have 
...
$$n = \# \calP = |\calX_{a,b}(\fqm)| \leq q^m-1+2g_{a,b}\sqrt{q^m},$$
...
$$\frac{m}{2}\left(2s'(q^{e^*}-q^{e^*-1}+1)+k^2(m-1-2e^*)  \right)\geq q^m-1+2g_{a,b}\sqrt{q^m},$$
