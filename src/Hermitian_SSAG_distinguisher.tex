...
\end{definition}%
%
Such a code has length $n = \# \calP$. By \eqref{eq:dim_ssag}, its dimension satisfies 
\[\dim_{\fq} \Gamma(\calP,D,g) \geq n-m \dim_{\fqm} \calC_{\calL}(\calX,\calP,D+(g)) =n-m(\deg D - \mathfrak{g} +1 )\]
if $2\mathfrak{g} - 2 < \deg D < n$. Its minimum distance is bounded from below by $\deg D - 2 \frak{g} + 2$.
