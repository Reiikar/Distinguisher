...
\begin{itemize}
    ...
    \item[$\star$] Finally, if $s(q^k+1) < a \leq (s+1)(q^k+1)$, then we can write $a = (s+1)q^k + \alpha$ with
...
\[A^k := \left\{(i,j) \ | \ x^iy^j \in \calL((q^k+1)sP_{\infty})\right\},\]
\[S^k := \left\{(i,j) \ | \ x^iy^j \in \calL(sP_{\infty})\right\}\]
and
\begin{align*}
...
\end{align*}
where 
\[ S^k_{(i_2,j_2)} := \left\{(i_1+q^ki_2,j_1+q^kj_2) \ | \ (i_1,j_1) \in S^k\right\}=S^k+q^k(i_2,j_2)\]
is the shift of the set $S^k$ by $(q^ki_2,q^kj_2)$. In particular, $S^k_{(0,0)}=S^k$.

Our goal is to prove that any $(i,j) \in A^k$ is in $B^k$; or more precisely, in some $S^k_{(i_2,j_2)}$. This will show that any element in the basis of $\calL((q^k+1)sP_{\infty})$ is also in the product $\calL(sP_{\infty}) \cdot \calL(sP_{\infty})^{q^k}$, finishing the proof.
...
we deduce that $i_{S^k} = \left\lfloor \dfrac{s}{q_0} \right\rfloor = q_0-1$ and $i_{A^k} = \left\lfloor \dfrac{s(q^k+1)}{q_0} \right\rfloor = (q_0-1)(q^k+1)$. To determine $j_{S^k}$ and $j_{A^k}$ we need to compare $\left\lfloor \dfrac{s}{q_0+1}\right\rfloor$ and $\left\lfloor \dfrac{s(q^k+1)}{q_0+1} \right\rfloor$ to $q_0-1$. We get
\begin{align*}
...
\end{equation}
the last inequality holding because $q_0 \geq 4$ (\emph{i.e.} $q \geq 2$). This gives $j_{A^k} = q_0-1$. Let us notice that
\[s(q^k+1) = i_{A^k}q_0+(q^{2k}-1) \quad \mathrm{and} \quad s=i_{S^k}q_0 + (q^k-1).\]
\jade{$\uparrow$ En a-t-on besoin ?}
