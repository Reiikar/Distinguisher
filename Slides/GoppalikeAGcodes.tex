...
\begin{frame}
	...
	Examples: elliptic curves, Hermitian curve...
	
	\begin{itemize}
		\item Its genus is $\mathfrak{g}_{a,b}=\dfrac{(a-1)(b-1)}{2}.$
		\item It has a unique point at infinity, denoted by $P_{\infty}$.
		%Riemann--Roch space $\calL(s P_\infty)$ has an explicit basis as follows:
		\item Polynomials in $\fq[x,y]$ are functions with poles only at $P_\infty$. Define $\degab{x^iy^j}= ai+bj.$
